...
async function main() {
  console.log('Seeding ECCA Assessment Framework...');
  ...
  for (const domain of eccaFramework.domains) {
    // Check if domain exists by checking framework_id and order_index (assuming order is unique per framework)
    // But we don't have a unique constraint on (framework_id, order_index) or name.
    // We'll delete existing domains for this framework and recreate them to ensure sync.
    // Actually, let's try to find by name first.
    
    const existingDomain = await prisma.assessmentDomain.findFirst({
      where: {
    ...
    if (existingDomain) {
      ...
      console.log(`Updated domain: ${domain.name}`);
    } else {
