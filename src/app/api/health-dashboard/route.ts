...
export async function GET() {
  ...
  if (process.env.NEXT_PUBLIC_DEPLOYMENT_TIMESTAMP) {
    try {
      ...
      console.error('Error calculating deployment age:', _error);
    }
  }
    ...
    if (fs.existsSync(dataDir)) {
      ...
      for (const file of files) {
        try {
          ...
          console.error(`Error reading file ${file}:`, _error);
        }
      }
    }
  } catch (_error) {
    console.error('Error reading verification reports:', _error);
  }
