      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="w-full bg-blue-200 h-1.5 rounded-full mt-2">
            <div
              className={`h-1.5 rounded-full ${
                averageCompletionWeeks > 20 ? 'bg-red-500' : 'bg-blue-500'
              }`}
              style={{ width: `${Math.min((averageCompletionWeeks / 25) * 100, 100)}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="bg-green-50 p-4 rounded-lg border border-green-100">
          ...
          <div className="w-full bg-green-200 h-1.5 rounded-full mt-2">
            <div
              className={`h-1.5 rounded-full ${
                withinDeadlinePercent < 90 ? 'bg-red-500' : 'bg-green-500'
              }`}
              style={{ width: `${withinDeadlinePercent}%` }}
            ></div>
          </div>
        </div>
