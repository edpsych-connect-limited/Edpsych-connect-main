...
const MAINTENANCE_EXEMPT_PATHS = ["/maintenance", "/api", "/_next", "/favicon.ico"];
// =============================================================================
...
export async function proxy(request: NextRequest) {
  ...
  // These routes are expected at the root (e.g. `/robots.txt`, `/sitemap.xml`).
  // If we let next-intl handle them, it may redirect to `/<locale>/...`, causing
  // 404s when the locale-prefixed metadata routes are not defined.
  ...
  if (MAINTENANCE_MODE) {
    ...
    if (!isExemptPath) {
      ...
      logger.debug("[Maintenance] Path:", pathname, "DevParam:", devParam, "DevCookie:", devCookie);

      // If they have the dev cookie, allow through immediately
      if (devCookie === "true") {
        logger.debug("[Maintenance] Dev cookie found, allowing through");
        // Continue to normal proxy flow - do nothing here
      }
      // If they have the correct bypass key, set cookie and redirect to clean URL
      else if (devParam === DEV_BYPASS_KEY) {
        logger.debug("[Maintenance] Dev param matched, setting cookie");
      ...
      // No bypass - redirect to maintenance page
        ...
        logger.debug("[Maintenance] No bypass, redirecting to maintenance");
  ...
  if (pathname.startsWith("/api/")) {
    ...
    if (!payload) {
      logger.debug("[Proxy] Unauthorized API request:", pathname);
      logger.debug("[Proxy] Cookies:", request.cookies.getAll());
      return NextResponse.json(
        { error: "Unauthorized (Proxy)" },
        {
          status: 401,
          headers: {
