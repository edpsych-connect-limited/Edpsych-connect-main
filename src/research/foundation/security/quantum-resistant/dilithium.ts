 ...
 * 
 * This module implements the CRYSTALS-Dilithium digital signature algorithm,
 * which is one of the finalists in the NIST Post-Quantum Cryptography standardization
 * process. Dilithium is lattice-based and provides quantum-resistant digital signatures.
 * 
 * The implementation follows the specifications from the NIST submission and includes
 * support for different security levels (Dilithium2, Dilithium3, Dilithium5).
 */
...
/**
 * Class implementing the CRYSTALS-Dilithium digital signature algorithm.
 */
export class Dilithium {
  ...
  /**
   * Creates a new instance of the Dilithium signature algorithm.
   * 
   * @param securityLevel The security level to use (default: DILITHIUM3)
  ...
  public async generateKeyPair(): Promise<KeyPair> {
    ...
    const sizes = ALGORITHM_PARAMETER_SIZES.DILITHIUM[this.securityLevel];
  ...
  /**
   * Sign a message using the Dilithium digital signature algorithm.
   * 
  ...
  /**
   * Verify a signature using the Dilithium digital signature algorithm.
   * 
  ...
  /**
   * Change the security level of the Dilithium instance.
   * 
   * @param securityLevel The new security level to use
   */
  public setSecurityLevel(securityLevel: DilithiumSecurityLevel): void {
    this.securityLevel = securityLevel;
  }
