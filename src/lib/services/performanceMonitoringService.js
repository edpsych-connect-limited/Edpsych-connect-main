...
class PerformanceMonitoringService {
    ...
    async getPerformanceMetrics(options = {}) {
        try {
            ...
            endTime = new Date(), includeSystem = true, includeCustom = true } = options;
            const metrics = {
                period: {
                ...
                alerts: this.alerts.filter(alert => new Date(alert.timestamp) >= startTime &&
    ...
    async generatePerformanceReport(options = {}) {
        try {
            const { period = 'daily', // daily, weekly, monthly
            includeTrends = true, includeRecommendations = true } = options;
            const report = {
                period,
    ...
    async _collectMetrics() {
        try {
            ...
            const recentRequests = this.metrics.responseTimes.filter(r => new Date(r.timestamp) > new Date(Date.now() - 60000));
    ...
    _calculateApiMetrics(startTime, endTime) {
        const relevantResponses = this.metrics.responseTimes.filter(r => new Date(r.timestamp) >= startTime && new Date(r.timestamp) <= endTime);
    ...
    _getCustomMetrics(startTime, endTime) {
        ...
        for (const [name, metrics] of this.metrics.customMetrics) {
            const relevantMetrics = metrics.filter(m => new Date(m.timestamp) >= startTime && new Date(m.timestamp) <= endTime);
    ...
    _findSlowestEndpoint(responses) {
        ...
        responses.forEach(response => {
            ...
            endpointStats[response.endpoint].maxTime = Math.max(endpointStats[response.endpoint].maxTime, response.responseTime);
        });
    ...
    _calculateSystemHealth(systemMetrics) {
        ...
        if (systemMetrics.memory.usagePercent > 90)
            ...
            score -= 15;
        if (systemMetrics.loadAverage[0] > systemMetrics.cpu.count * 2)
            score -= 20;
        else if (systemMetrics.loadAverage[0] > systemMetrics.cpu.count)
            score -= 10;
        if (score >= 80)
            return 'healthy';
        if (score >= 60)
    ...
    _cleanupOldMetrics() {
        ...
        // Clean up old metrics
        this.metrics.responseTimes = this.metrics.responseTimes.filter(m => new Date(m.timestamp).getTime() > cutoffTime);
        this.metrics.memoryUsage = this.metrics.memoryUsage.filter(m => new Date(m.timestamp).getTime() > cutoffTime);
        this.metrics.cpuUsage = this.metrics.cpuUsage.filter(m => new Date(m.timestamp).getTime() > cutoffTime);
        // Clean up old alerts
