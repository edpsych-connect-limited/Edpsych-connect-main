...
export class ContentScraper {
  ...
  async scrapeAllSources(): Promise<ScrapedContent[]> {
    ...
    for (const source of CONTENT_SOURCES) {
      try {
        ...
        console.error(`[Content Scraper] Error scraping ${source.name}:`, _error);
      }
    }
  ...
  private async scrapeSource(source: ContentSource): Promise<ScrapedContent[]> {
    ...
    try {
      ...
      console.error(`[Content Scraper] Error processing ${source.name}:`, _error);
    }
  ...
  private async fetchArticles(source: ContentSource): Promise<any[]> {
    ...
    try {
      ...
      console.error(`[Content Scraper] Failed to fetch RSS for ${source.name}:`, _error);
      return this.getSimulatedArticles(source);
    }
  }
  ...
  async scrapeUrl(url: string): Promise<ScrapedContent | null> {
    try {
      ...
      console.error(`[Content Scraper] Error scraping ${url}:`, _error);
      return null;
    }
  }
}
