...
export class SENCODashboardService {
  ...
  async getCaseloadSummary(schoolId: string): Promise<CaseloadSummary> {
    ...
    students.forEach(entry => {
      // By status
      byStatus[entry.sendStatus] = (byStatus[entry.sendStatus] || 0) + 1;
      
      // By need type
      byNeedType[entry.primaryNeed] = (byNeedType[entry.primaryNeed] || 0) + 1;
      
      // By year group
      const yearGroup = (entry as unknown as { student: { yearGroup: string } }).student?.yearGroup || 'Unknown';
      byYearGroup[yearGroup] = (byYearGroup[yearGroup] || 0) + 1;
      
      // By RAG
      byProgressRAG[entry.progressRAG] = (byProgressRAG[entry.progressRAG] || 0) + 1;
      
      // Flags
      if (entry.requiresUrgentAction) urgentActions++;
      if (entry.nextReviewDate && new Date(entry.nextReviewDate) < now) overdueReviews++;
      if (entry.hasTransition) upcomingTransitions++;
    });
  ...
  async getSENDRegister(
    ...
    if (filters.yearGroup) {
      where.student = { yearGroup: filters.yearGroup };
    }
    ...
    const [entries, total] = await Promise.all([
      this.prisma.sENDRegister.findMany({
        where,
        include: {
          student: {
            select: {
    ...
    return {
      entries: entries.map(e => this.transformRegisterEntry(e)),
      total,
      page,
  ...
  async updateRegisterEntry(
    ...
    const updated = await this.prisma.sENDRegister.update({
      where: { id: entryId },
      data: {
        sendStatus: data.sendStatus,
        primaryNeed: data.primaryNeed,
        secondaryNeeds: data.secondaryNeeds,
  ...
  async addToRegister(
    ...
    const entry = await this.prisma.sENDRegister.create({
      data: {
        schoolId,
        studentId,
        sendStatus: data.sendStatus,
        ...
        identificationReason: data.reason,
        status: 'ACTIVE'
      },
      include: {
        student: true,
  ...
  async getComplianceStatus(schoolId: string): Promise<ComplianceStatus> {
    ...
    // EHCP compliance
    const ehcpStudents = await this.prisma.sENDRegister.findMany({
      where: { schoolId, sendStatus: { in: ['EHCP', 'EHCP_ASSESSMENT'] } },
      include: { ehcp: true }
    });

    const ehcpCompliance = {
      total: ehcpStudents.filter(s => s.sendStatus === 'EHCP').length,
      compliant: 0,
      annualReviewsDue: 0,
      annualReviewsOverdue: 0,
      assessmentsInProgress: ehcpStudents.filter(s => s.sendStatus === 'EHCP_ASSESSMENT').length,
      assessmentsOverdue: 0
    };

    ehcpStudents.forEach(student => {
    ...
    const senSupportStudents = await this.prisma.sENDRegister.findMany({
    ...
    senSupportStudents.forEach(student => {
    ...
    const [provisionMaps, outcomes, consents, riskAssessments] = await Promise.all([
      ...
      this.prisma.sENDOutcome.count({
        where: { schoolId, lastUpdated: { gte: new Date(now.getFullYear(), now.getMonth() - 1, 1) } }
      }),
  ...
  async getStatutoryDeadlines(schoolId: string): Promise<StatutoryDeadline[]> {
    ...
    assessments.forEach(assessment => {
      const startDate = new Date(assessment.requestDate);
  ...
  async getActionItems(
    ...
    return items.map(item => ({
      id: item.id,
      type: item.type as ActionItem['type'],
      priority: item.priority as ActionItem['priority'],
      title: item.title,
      description: item.description,
      studentId: item.studentId || undefined,
      studentName: item.studentId ? `${(item as unknown as { student: { firstName: string; lastName: string } }).student.firstName} ${(item as unknown as { student: { firstName: string; lastName: string } }).student.lastName}` : undefined,
      dueDate: item.dueDate,
      status: item.status as ActionItem['status'],
      assignedTo: item.assignedToId || undefined,
  ...
  private async getWeeklyActivity(schoolId: string): Promise<DashboardMetrics['weeklyActivity']> {
    ...
    const [reviews, parentMeetings, profMeetings, documents, interventions] = await Promise.all([
      ...
      this.prisma.intervention.count({
        where: { schoolId, startDate: { gte: weekAgo } }
      })
    ]);
  ...
  private async getResourceSummary(schoolId: string): Promise<DashboardMetrics['resourceSummary']> {
    const [provisionCost, staffHours, externalHours] = await Promise.all([
      ...
      this.prisma.staffAllocation.aggregate({
        where: { schoolId, status: 'ACTIVE' },
        _sum: { hoursPerWeek: true }
      }),
      this.prisma.externalSpecialist.aggregate({
        where: { schoolId, status: 'ACTIVE' },
        _sum: { hoursPerWeek: true }
      })
    ]);
    ...
    const budget = await this.prisma.schoolBudget.findFirst({
